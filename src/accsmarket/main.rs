...
async fn main() -> anyhow::Result<()> {
    ...
    let res = client.get("https://accsmarket.com/").send().await?.text().await?;
    ...
    let sel_scp = scraper::Selector::parse(".soc-text>p").unwrap();
    let container = html.select(&sel_soc_bl).next().ok_or_else(|| anyhow::anyhow!("element not found"))?;

    let ctx = scrape::Context { client, sel_scp };
